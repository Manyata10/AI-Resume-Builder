...
const MinimalImageTemplate = ({ data, accentColor }) => {
    ...
    return (
        <div className="max-w-5xl mx-auto bg-white text-zinc-800 text-[13px] leading-snug">
                    ...
                    <section className="mb-8">
                        ...
                        <div className="space-y-3 text-sm">
                            ...
                            {data.personal_info?.linkedin && (
                                <a href={data.personal_info.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                                    <Linkedin size={14} style={{ color: accentColor }} />
                                    <span>LinkedIn</span>
                                </a>
                            )}
                            {data.personal_info?.github && (
                                <a href={data.personal_info.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                                    <Github size={14} style={{ color: accentColor }} />
                                    <span>GitHub</span>
                                </a>
                            )}
                            {data.personal_info?.leetcode && (
                                <a href={data.personal_info.leetcode} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                                    <Code size={14} style={{ color: accentColor }} />
                                    <span>LeetCode</span>
                                </a>
                            )}
                            {data.personal_info?.website && (
                                <a href={data.personal_info.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                            ...
                            <div className="space-y-4 text-sm">
                                {data.education.map((edu, index) => (
                                    <div key={index}>
                                        ...
                                        {edu.gpa && <p className="text-xs font-medium text-zinc-700 mt-0.5">CGPA: {edu.gpa}</p>}
                                    </div>
                                ))}
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="space-y-3 text-sm">
                                {data.skills.map((skill, index) => (
                                    <div key={index}>
                                        <p className="font-semibold text-zinc-700">{skill.title || "Skill"}</p>
                                        <p className="text-zinc-600">{(skill.items ? skill.items : [skill]).join(", ")}</p>
                                    </div>
                                ))}
                            </div>
                        </section>
